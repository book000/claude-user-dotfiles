...
2. コミット内容にセンシティブな情報が含まれていないこと
3. コンフリクトする恐れが無いこと
4. `/code-review:code-review` によるコードレビューを実施したこと。ただしコードレビュー前にコミットしてください。`/code-review:code-review` へは、ローカルブランチの変更内容についてレビューを行うことを明示的に指定してください。
...
4. 10分以内に投稿される GitHub Copilot レビューへの対応を行うこと
