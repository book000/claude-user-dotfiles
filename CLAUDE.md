...
### コミュニケーション
- **日本語優先**: すべての会話は日本語で行う
- **簡潔性**: 前置きや不要な説明は避け、直接的に回答
- **具体性**: 抽象的な説明よりも具体的なコード例や手順を提示

### コード品質とパターン
- **既存規約の尊重**: プロジェクトの既存コード規約・アーキテクチャに従う
- **依存関係の確認**: 新しいライブラリ使用前に package.json で既存依存関係を必ず確認
- **セキュリティ重視**: 認証情報や機密データの漏洩を防ぐ

### 学習とルール管理
- **継続指示の標準化**: 一回限りでない指示は「これを標準ルールにしますか？」と確認
- **ルール追加**: YES回答時はCLAUDE.mdに追加し、以降の標準として適用
...
### ファイル操作の優先順位
1. **Read** → 既存ファイル内容の確認（必須）
2. **Edit** → 部分的な修正（推奨）
3. **MultiEdit** → 同一ファイル内の複数箇所修正
4. **Write** → 新規ファイル作成（最小限・明示的要求時のみ）
...
### 検索戦略の効率化
1. **Task Tool（推奨）**: 複雑な調査・複数ファイルにわたる検索
2. **Grep Tool**: 特定パターンの高速検索
3. **Glob Tool**: ファイル名パターンでの検索
4. **並行実行**: 独立した検索を同時実行でパフォーマンス向上

### 大量データ検証
多数ファイル参照時は一時的な検証スクリプトを作成・実行（コミットしない）:
...
- **ファイル作成の抑制**: 絶対に必要な場合以外は既存ファイルの編集を優先
- **ドキュメント作成の禁止**: ユーザーが明示的に要求しない限り、*.md や README ファイルは作成しない
- **セキュリティ最優先**: 認証情報やAPIキーの漏洩を絶対に防ぐ
